...
def get_parser(desc, default_task='translation'):
    ...
    usr_parser = argparse.ArgumentParser(add_help=False, formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def add_dataset_args(parser, train=False, gen=False):
    ...
    if train:
        ...
        group.add_argument('--token-labeling-positive-label-weight', default=1, type=float,
                           help='token labeling positive label weight')
    if gen:
...
def add_generation_args(parser):
    ...
    group.add_argument('--score-hypotheses', action='store_true',
                       help='score the hypotheses')
    # fmt: on
    return group
